...
export class DecoratorBinder extends BinderBase {
  bind(
    ...
    let decorators = routeInfo.methodMetaData!.parameters.filter(
      x => x.name == parameterName
    )[0].decorators;
    if (!decorators) return this.next();

    let decorator = decorators.filter(
      x => x.name == "body" || x.name == "cookie"
    )[0];
    ...
    if (decorator.name == "cookie") {
      let value = convert(
        routeInfo,
        parameterName,
        this.extractCookie(request, decorator)
      );
