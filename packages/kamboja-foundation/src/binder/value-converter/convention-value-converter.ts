...
export class ConventionValueConverter extends BaseConverter {
  convert(meta: Kecubung.ParameterMetaData, value: string): ConverterResult {
    ...
    if (
      (meta.name.indexOf("i") == 0 || meta.name.indexOf("n") == 0) &&
  ...
  toString(value: any, meta: Kecubung.ParameterMetaData) {
    if (typeof value == "object")
      throw new Error(
        `Expected parameter type of 'string'  but got object in [${meta.name}] ${getRouteDetail(
          this.routeInfo
        )}`
      );
  ...
  toNumber(value: any, meta: Kecubung.ParameterMetaData) {
    if (typeof value == "object")
      throw new Error(
        `Expected parameter type of 'number'  but got object in [${meta.name}] ${getRouteDetail(
          this.routeInfo
        )}`
      );
  ...
  toBoolean(value: any, meta: Kecubung.ParameterMetaData) {
    if (typeof value == "object")
      throw new Error(
        `Expected parameter type of 'boolean'  but got object in [${meta.name}] ${getRouteDetail(
          this.routeInfo
        )}`
      );
