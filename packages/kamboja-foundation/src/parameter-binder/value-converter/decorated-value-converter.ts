...
export class DecoratedValueConverter extends BaseConverter {
  convert(meta: Kecubung.ParameterMetaData, value: string): ConverterResult {
    let decorators =
      meta.decorators && meta.decorators.filter(x => this.filterTypes(x));
    ...
    if (typeof value == "object" && !Array.isArray(value))
      throw new Error(
        `Expected parameter type of [@type('${type}') ${meta.name}] but got object in ${getRouteDetail(
          this.routeInfo
        )}`
      );
    switch (type) {
  ...
  filterTypes(x: Kecubung.DecoratorMetaData) {
    return (
      x.name == "type" &&
      ValidationTypesAccepted.some(y => this.getParameter(x) == y)
    );
  }

  getParameter(x: Kecubung.DecoratorMetaData) {
    return (<Kecubung.PrimitiveValueMetaData>x.parameters[0]).value
      .toString()
      .toLowerCase();
  }
}
