...
describe("Binder.ParameterBinder", () => {
    ...
    describe("Default", () => {
        ...
        it("Should bind string value properly", () => {
            ...
            let binder = new Binder.ParameterBinder(infos.filter(x => x.methodMetaData!.name == "myMethod")[0], pathResolver);
        ...
        it("Should return empty array for method without parameter", () => {
            ...
            getParamStub.withArgs("par1").returns("false")
            getParamStub.withArgs("par2").returns("True")
            let binder = new Binder.ParameterBinder(infos.filter(x => x.methodMetaData!.name == "noParam")[0], pathResolver);
            let result = binder.getParameters(HttpRequest)
        ...
        it("Should return empty array for method without parameter", () => {
            ...
            getParamStub.withArgs("iage").returns("30")
            getParamStub.withArgs("bisdirty").returns("True")
            let binder = new Binder.ParameterBinder(infos.filter(x => x.methodMetaData!.name == "conventionConversion")[0], pathResolver);
            let result = binder.getParameters(HttpRequest);
    ...
    describe("Api Convention", () => {
        ...
        it("Should fallback to default binding on GET Http Method", () => {
            ...
            let binder = new Binder.ParameterBinder(infos.filter(x => x.methodMetaData!.name == "list")[0], pathResolver);
            let result = binder.getParameters(HttpRequest)
        ...
        it("Should assign HttpRequest Body on first parameter on POST method", () => {
            ...
            let meta = H.fromFile("controller/parameter-binder-controller.js", pathResolver)
            let infos = Router.transform(meta)
            let binder = new Binder.ParameterBinder(infos.filter(x => x.methodMetaData!.name == "add")[0], pathResolver);
            let result = binder.getParameters(HttpRequest)
        ...
        it("Should assign HttpRequest Body on second parameter on PUT method", () => {
            ...
            let meta = H.fromFile("controller/parameter-binder-controller.js", pathResolver)
            let infos = Router.transform(meta)
            let binder = new Binder.ParameterBinder(infos.filter(x => x.methodMetaData!.name == "modify")[0], pathResolver);
    ...
    describe("Decorator", () => {
        ...
        it("Should bind request body properly", () => {
            ...
            let binder = new Binder.ParameterBinder(infos.filter(x => x.methodMetaData!.name == "decoratorBinder")[0], pathResolver);
            let result = binder.getParameters(HttpRequest)
    ...
    describe("@type() decorator binder", () => {
        it("Should bind body properly", () => {
            ...
            let binder = new Binder.ParameterBinder(infos.filter(x => x.methodMetaData!.name == "typeBinder")[0], pathResolver);
            let result = binder.getParameters(HttpRequest)
            Chai.expect(result).deep.eq([{ data: "Hello!" }])
        })
        ...
        it("Should not bind parameter if @bind.body() provided", () => {
            ...
            let binder = new Binder.ParameterBinder(infos.filter(x => x.methodMetaData!.name == "typeBinderWithBind")[0], pathResolver);
            let result = binder.getParameters(HttpRequest)
        ...
        it("Should not bind @type() with non qualified class name", () => {
            ...
            let binder = new Binder.ParameterBinder(infos.filter(x => x.methodMetaData!.name == "typeBinderWithOtherType")[0], pathResolver);
            let result = binder.getParameters(HttpRequest)
            Chai.expect(result).deep.eq([undefined, { data: "Hello!" }])
        })
    })
